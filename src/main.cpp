...
extern const unsigned int nTargetSpacing = 1 * 60; // 60 seconds
...
const string strMessageMagic = "Fusecoin Signed Message:\n";
...
static const int64_t nTargetTimespan = 10 * nTargetSpacing;  // ~ 10 blocks
//
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016"PRIx64", expected=0x%x",
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Fusecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Ukrainian Rebels Hand Over Black Boxes to Malaysian Investigators";
        CTransaction txNew;
        txNew.nTime  = 1405999455;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1405999455;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2774895;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        if (false && (block.GetHash() != hashGenesisBlock)
        		&& (block.GetHash() > CBigNum().SetCompact(block.nBits).getuint256())) {
        ...
        //// debug print
        //assert(block.hashMerkleRoot == uint256("0xf03a02af48a7e5833952293d439ae72247d5dd2462d442a0bd98de7bd78b2c6c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa5, 0xa0, 0xa2, 0xa6 };
